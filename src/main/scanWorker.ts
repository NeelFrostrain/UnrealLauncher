...
interface Engine {
  version: string; exePath: string; directoryPath: string
  folderSize: string; lastLaunch: string; gradient: string
}

interface Project {
  name: string; version: string; size: string; createdAt: string
  lastOpenedAt?: string; projectPath: string; thumbnail: string | null; projectId?: string
}
...
function scanEnginePaths(): Array<{ version: string; exePath: string; directoryPath: string }> {
  if (native) { try { return native.scanEngines([]) } catch { /* fall through */ } }
...
function findUprojectFiles(dir: string, maxDepth = 5, maxFiles = 1000): string[] {
  if (native) { try { return native.findUprojectFiles(dir, maxDepth, maxFiles) } catch { /* fall through */ } }
...
function findProjectScreenshot(p: string): string | null {
  if (native) { try { return native.findProjectScreenshot(p) ?? null } catch { /* fall through */ } }
...
function findLatestLogTimestamp(p: string): string | null {
  if (native) { try { return native.findLatestLogTimestamp(p) ?? null } catch { /* fall through */ } }
...
function runScanEngines(saved: Engine[]): Engine[] {
  const scanned: Engine[] = scanEnginePaths().map((e) => {
    ...
    return {
      version: e.version, exePath: e.exePath, directoryPath: e.directoryPath,
...
function runScanProjects(saved: Project[]): Project[] {
  ...
  return merged.filter((p) => p.projectPath && fs.existsSync(path.join(p.projectPath, `${p.name}.uproject`)))
}
