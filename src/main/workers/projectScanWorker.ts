...
async function findUprojectFiles(dir) {
  if (native) { try { return await native.findUprojectFiles(dir, SCAN_MAX_DEPTH, SCAN_MAX_FILES); } catch {} }
  const files = []; let count = 0;
  const SKIP = new Set(['node_modules','.git','Binaries','Intermediate','DerivedDataCache','Saved','Plugins']);
  async function scan(cur, depth) {
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (isDir && !item.startsWith('.') && !SKIP.has(item)) {
          await scan(full, depth+1);
        } else if (!isDir && item.endsWith('.uproject')) {
          files.push(full);
          count++;
        }
      }
...
async function run() {
  ...
  for (const sp of searchPaths) {
    ...
    const freshProjects = [];
    const foundFiles = await findUprojectFiles(sp);
    for (const up of foundFiles) {
      try {
