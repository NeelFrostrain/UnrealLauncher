...
/**
 * Gets Windows-specific system information (JS fallback only)
 */
...
/**
 * Gets primary network interfaces (JS fallback)
 */
function getNetworkInterfacesJS(): Array<{ name: string; ipv4: string }> {
      ...
      if (ipv4 && !ipv4.address.startsWith('127.')) {
        ...
        if (isPrivate) {
          if (!ipv4.address.startsWith('192.168.5')) {
            internal.push({ name, ipv4: ipv4.address })
          }
        } else {
          external.push({ name, ipv4: ipv4.address })
        }
      }
    }
  }
...
/**
 * Gets disk information (JS fallback, Windows only)
 */
function getDiskInfoJS(): Array<{
  drive: string
  total: string
  used: string
  free: string
...
/**
 * Gets GPU information (JS fallback, Windows only)
 */
function getGpuInfoJS(): string {
  if (process.platform !== 'win32') return 'Not available on this platform'
  try {
 ...
 * Delegates hardware/OS collection to the Rust native module when available,
 * falling back to the JS implementation if native is not loaded.
 */
export async function getSystemInfo(appVersion: string): Promise<SystemInfo> {
  ...
  if (native?.getSystemHardwareInfo) {
    try {
      const hw = native.getSystemHardwareInfo()
      ...
      return {
        appVersion,
        pcName: hw.hostname,
        platform: hw.platform || platformMap[process.platform] || process.platform,
        platformVersion: os.release(),
        platformBuild: hw.platformBuild ?? undefined,
        arch: hw.arch,
        cpuModel: hw.cpuModel,
        cpuCores: hw.cpuCores,
        cpuThreads: hw.cpuThreads,
        cpuSpeed: hw.cpuSpeedMhz,
        totalMemory: formatBytes(hw.totalMemoryBytes),
        freeMemory: formatBytes(hw.freeMemoryBytes),
        usedMemory: formatBytes(hw.totalMemoryBytes - hw.freeMemoryBytes),
        hostname: hw.hostname,
        uptime: formatUptime(hw.uptimeSecs),
        networkInterfaces,
        username: hw.username,
        osType: hw.osType ?? undefined,
        gpuInfo: hw.gpuInfo ?? undefined,
        diskInfo,
  ...
  return {
    ...
    networkInterfaces: getNetworkInterfacesJS(),
    username: process.env.USERNAME || process.env.USER || 'Unknown',
    osType: windowsInfo.osType,
    gpuInfo: getGpuInfoJS(),
    diskInfo: getDiskInfoJS(),
    nodeVersion: process.version,
    timezone: locationInfo.timezone,
    locale: locationInfo.locale,
    publicIp
