...
import { getEngineInstallPaths, getBinaryExtension } from '../system/platformPaths'
...
export async function scanEnginePaths(extraPaths: string[] = []): Promise<ScannedEngine[]> {
  ...
  // Prefer Rust for default scan paths — falls back to platformPaths.ts if native unavailable
  const defaultPaths = native?.getDefaultEngineScanPaths?.() ?? getEngineInstallPaths()
  const allPaths = [...new Set([...defaultPaths, ...allExtra])]

  if (native) {
    try {
      const results = await native.scanEngines(allPaths)
  ...
  const results = await _scanEnginesJS(allPaths)
