...
export function generateGradient(): string {
  ...
  if (native?.generateEngineGradientNative) {
    try {
      return native.generateEngineGradientNative()
    } catch {
      /* fallback */
    }
  }
...
export function compareVersions(a: string, b: string): boolean {
  const native = getNative()
  if (native?.compareSemverVersionsNative) {
    try {
      return native.compareSemverVersionsNative(a, b) > 0
    } catch {
      /* fallback */
    }
  }
