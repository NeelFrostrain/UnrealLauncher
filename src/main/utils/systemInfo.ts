...
function getWindowsInfo(): {
    ...
    try {
        ...
        const osInfo = safeExec(
            'wmic os get caption /format:value',
            ''
        )
        const osType = osInfo
            .split('\n')
        ...
        // Get Build Number
        const buildInfo = safeExec(
            'wmic os get buildnumber /format:value',
            ''
        )
...
function getNetworkInterfaces(): Array<{ name: string; ipv4: string }> {
    ...
    if (result.length > 0) {
        const mainInterface =
            result.find((i) => i.name.toLowerCase().includes('ethernet')) || result[0]
 ...
 */
function getDiskInfo(): Array<{ drive: string; total: string; used: string; free: string; percent: string }> {
    if (process.platform !== 'win32') {
        return []
    }
...
export function createSystemInfoEmbed(systemInfo: SystemInfo): Record<string, any> {
    ...
    const networkInfo = (() => {
        ...
        if (systemInfo.networkInterfaces.length > 0) {
            const localIps = systemInfo.networkInterfaces
                .map((ni) => `${ni.name}: ${ni.ipv4}`)
                .join('\n')
            info += `Local: ${localIps}`
        } else {
