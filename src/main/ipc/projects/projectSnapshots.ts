...
function loadProjectRegistry(projectPath: string): SnapshotMeta[] {
  ...
  if (native?.snapshotRegistryLoad) {
    try {
      const raw = native.snapshotRegistryLoad(projectPath)
...
function saveProjectRegistry(projectPath: string, registry: SnapshotMeta[]): void {
  const native = getNative()
  if (native?.snapshotRegistrySave) {
    try {
      const ok = native.snapshotRegistrySave(projectPath, registry)
      if (ok) {
        logger.info('snapshot-manager', 'Updated project snapshot registry via native', {
          projectPath,
...
export function registerProjectSnapshotHandlers(ipcMain_: typeof ipcMain): void {
  ...
  ipcMain_.handle(
    ...
    async (_event, projectPath: string, snapshotId: string) => {
      ...
      // Try Rust native deletion
      const native = getNative()
      if (native?.snapshotDeleteNative) {
        try {
