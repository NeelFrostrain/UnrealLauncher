...
export async function scanProjectPlugins(projectPath: string): Promise<ProjectPlugin[]> {
  ...
  if (native?.scanProjectPlugins) {
    try {
      const nativePlugins = native.scanProjectPlugins(projectPath)
      plugins = nativePlugins.map((p) => ({
        name: p.name,
  ...
  if (!plugins) {
    // Use worker fallback if native module is unavailable
    plugins = await scanProjectPluginsJS(projectPath)
  }
