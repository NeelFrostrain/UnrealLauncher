...
export function handleProjectCppCreateStructure(
  ...
  // Try Rust native generator first
  const native = getNative()
  if (native?.createCppSourceStructure) {
    try {
      const res = native.createCppSourceStructure(safePath)
      if (res.success) {
        logger.info('project-cpp', 'Created C++ source structure via native', {
          projectPath,
          createdFiles: res.createdFiles
        })
        return { success: true, createdFiles: res.createdFiles }
      } else if (res.error) {
...
async function findRiderExe(customPath?: string): Promise<string | null> {
  ...
  // Try Rust native discovery first
  const native = getNative()
  if (native?.findRiderExecutable) {
    try {
      const found = native.findRiderExecutable(customPath)
      if (found) {
        _cachedRiderExe = found
        return found
      }
    } catch {
      /* fallback to JS */
...
function findVisualStudioExe(): string | null {
  ...
  // Try Rust native discovery first
  const native = getNative()
  if (native?.findVisualStudioExecutable) {
    try {
      const found = native.findVisualStudioExecutable()
      if (found) {
        _cachedVsExe = found
        return found
      }
