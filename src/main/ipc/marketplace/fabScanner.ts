...
import { getFabCachePaths, createFabAsset, type FabAsset, getNative } from '../../utils'
...
export async function scanFabFolder(rootDir: string): Promise<FabAsset[]> {
  logger.info('fab-scanner', 'Scanning Fab vault cache directory', { rootDir })
  const assets: FabAsset[] = []
  try {
    await fsPromises.access(rootDir)
  } catch {
    logger.warn('fab-scanner', 'Fab cache directory not accessible', { rootDir })
    return assets
  }
