...
import { setEnginesCache } from '../engines/useEngineCompatibility'
  ...
  const refreshEngineOptions = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      for (const engine of engines) {
        const raw = (engine.version ?? '').trim()
        if (raw && raw.toLowerCase() !== 'unknown') {
          const mm = toMajorMinorVersion(raw)
          versions.add(mm)
        }
      }
      for (const p of allProjectsRef.current) {
        const raw = (p.version ?? '').trim()
        if (raw && raw.toLowerCase() !== 'unknown' && !raw.startsWith('{')) {
          versions.add(toMajorMinorVersion(raw))
        }
      }
      ...
      for (const version of [...versions].sort((a, b) =>
        ...
        nextOptions.push({ value: version, label: version })
      }
