...
import {
  Search, Zap, Package, Settings, RefreshCw, Plus,
  Star, EyeOff, Clock, FolderOpen, Terminal, ChevronRight, X,
  Play, Gamepad2
} from 'lucide-react'
...
const STATIC_COMMANDS: PaletteItem[] = [
  { id: 'nav-engines',            kind: 'command', label: 'Go to Engines',          icon: <Zap size={13} style={{ color: '#818cf8' }} />,                        group: 'Navigate', shortcut: `${MOD}+1` },
  { id: 'nav-projects',           kind: 'command', label: 'Go to Projects',          icon: <Package size={13} style={{ color: 'var(--color-accent)' }} />,        group: 'Navigate', shortcut: `${MOD}+2` },
  { id: 'nav-projects-recent',    kind: 'command', label: 'Recent Projects',          icon: <Clock size={13} style={{ color: 'var(--color-text-muted)' }} />,      group: 'Navigate' },
  { id: 'nav-projects-favorites', kind: 'command', label: 'Favorite Projects',        icon: <Star size={13} style={{ color: '#facc15' }} />,                       group: 'Navigate' },
  { id: 'nav-projects-hidden',    kind: 'command', label: 'Hidden Projects',          icon: <EyeOff size={13} style={{ color: 'var(--color-text-muted)' }} />,     group: 'Navigate' },
  { id: 'nav-engines-plugins',    kind: 'command', label: 'Engine Plugins',           icon: <Terminal size={13} style={{ color: 'var(--color-text-muted)' }} />,   group: 'Navigate' },
  { id: 'nav-engines-fab',        kind: 'command', label: 'Fab Cache',                icon: <FolderOpen size={13} style={{ color: 'var(--color-text-muted)' }} />, group: 'Navigate' },
  { id: 'nav-settings',           kind: 'command', label: 'Go to Settings',           icon: <Settings size={13} style={{ color: 'var(--color-text-muted)' }} />,   group: 'Navigate', shortcut: `${MOD}+3` },
  { id: 'action-refresh',         kind: 'command', label: 'Refresh Scan',             description: 'Re-scan engines or projects',                                   icon: <RefreshCw size={13} style={{ color: 'var(--color-text-muted)' }} />, group: 'Actions', shortcut: `${MOD}+R` },
  { id: 'action-add-project',     kind: 'command', label: 'Add Project',              description: 'Browse for a .uproject folder',                                 icon: <Plus size={13} style={{ color: 'var(--color-accent)' }} />,         group: 'Actions', shortcut: `${MOD}+N` },
  { id: 'action-add-engine',      kind: 'command', label: 'Add Engine',               description: 'Browse for an engine installation',                              icon: <Plus size={13} style={{ color: '#818cf8' }} />,                    group: 'Actions' },
  { id: 'action-search-projects', kind: 'command', label: 'Search Projects',          description: 'Focus the project search bar',                                  icon: <Search size={13} style={{ color: 'var(--color-text-muted)' }} />,  group: 'Actions', shortcut: `${MOD}+F` },
]
...
function scoreItem(item: PaletteItem, q: string): number {
  return scoreText(q, item.label) * 2
...
const S = {
  root: {
    ...
    width: '100vw', height: '100vh',          // own the full viewport — no parent dependency
  ...
  kbd: {
    padding: '1px 6px', borderRadius: 4, fontFamily: 'monospace', fontSize: 10,
    backgroundColor: 'var(--color-surface)',
    border: '1px solid var(--color-border)',
    color: 'var(--color-text-muted)'
  },
  closeBtn: {
    padding: 4, cursor: 'pointer', background: 'none', border: 'none',
  ...
  footerKbd: {
    padding: '0 4px', borderRadius: 3, fontFamily: 'monospace', fontSize: 10,
    backgroundColor: 'var(--color-surface)',
    border: '1px solid var(--color-border)'
  }
} as const
...
export function PaletteWindow(): React.ReactElement {
  ...
  useEffect(() => {
    window.paletteAPI?.getData()
      .then(({ engines: e, projects: p }) => { setEngines(e ?? []); setProjects(p ?? []) })
      .catch(() => {})
  ...
  const onKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    if      (e.key === 'ArrowDown') { e.preventDefault(); setActiveIdx((i) => (i + 1) % n) }
    else if (e.key === 'ArrowUp')   { e.preventDefault(); setActiveIdx((i) => (i - 1 + n) % n) }
    else if (e.key === 'Enter')     { e.preventDefault(); if (filtered[activeIdx]) run(filtered[activeIdx]) }
    else if (e.key === 'Escape')    { e.preventDefault(); window.paletteAPI?.close() }
  ...
  const grouped = useMemo(() => {
    ...
    filtered.forEach((item, idx) => {
      if (!seen.has(item.group)) { seen.add(item.group); out.push({ group: item.group, items: [] }) }
    ...
    <div style={S.root}>
      ...
      <div style={S.searchBar}>
        ...
        <div style={S.searchRight}>
          ...
          <button
            style={S.closeBtn}
            onClick={() => window.paletteAPI?.close()}
            aria-label="Close"
          >
            <X size={13} />
          </button>
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, marginLeft: 'auto' }}>
                    {active && isLaunchable && (
                      <span style={{
                        ...
                        {item.kind === 'engine'
                          ? <><Play size={9} />Launch</>
                          : <><Gamepad2 size={9} />Open</>
                        }
                      </span>
                    )}
                    {item.shortcut && !active && (
                      <kbd style={S.kbd}>{item.shortcut}</kbd>
                    )}
                    {active && !isLaunchable && (
      ...
      {/* ── Footer ── */}
      <div style={S.footer}>
        {([['↑↓', 'navigate'], ['↵', 'run'], ['Esc', 'close']] as const).map(([key, label]) => (
