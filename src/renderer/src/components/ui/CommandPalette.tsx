...
import {
  ...
  Star, EyeOff, Clock, FolderOpen, X, Terminal, ChevronRight
} from 'lucide-react'
...
export function CommandPalette({
  open, onClose, onRefresh, onAddProject, onAddEngine, onFocusSearch
  ...
  const grouped = useMemo(() => {
    ...
    filtered.forEach((cmd, globalIdx) => {
      if (!seen.has(cmd.group)) { seen.add(cmd.group); out.push({ group: cmd.group, commands: [] }) }
          ...
          >
            ...
            <div className="flex items-center gap-3 px-4 py-3 shrink-0"
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <kbd className="text-[10px] px-1.5 py-0.5 rounded font-mono" style={kbdStyle}>Esc</kbd>
                <button onClick={onClose} className="p-1 cursor-pointer" aria-label="Close" style={{ color: 'var(--color-text-muted)' }}>
                  <X size={13} />
                </button>
              </div>
            </div>
            ...
            <div ref={listRef} role="listbox" aria-label="Commands"
              ...
              {filtered.length === 0 ? (
                ...
                <div key={group}>
                  <p className="px-4 pt-2 pb-1 text-[10px] font-semibold uppercase tracking-widest select-none"
                    style={{ color: 'var(--color-text-muted)', opacity: 0.5 }}>{group}</p>
                  {commands.map(({ globalIdx, ...cmd }) => {
                    const active = activeIdx === globalIdx
                    return (
                      <button key={cmd.id} role="option" aria-selected={active} data-idx={globalIdx}
                        onClick={() => run(cmd)} onMouseEnter={() => setActiveIdx(globalIdx)}
                        className="w-full flex items-center gap-3 px-4 py-2.5 text-sm cursor-pointer transition-colors text-left"
                        style={{
                          backgroundColor: active ? 'color-mix(in srgb, var(--color-accent) 10%, var(--color-surface-card))' : 'transparent',
                          color: active ? 'var(--color-text-primary)' : 'var(--color-text-secondary)'
                        }}>
                        <span className="shrink-0 w-5 flex items-center justify-center">{cmd.icon}</span>
                        <span className="flex-1 min-w-0">
                          <span className="block font-medium leading-snug truncate">{cmd.label}</span>
                          {cmd.description && (
                            <span className="block text-[11px] leading-snug mt-0.5 truncate"
                              style={{ color: 'var(--color-text-muted)' }}>{cmd.description}</span>
                          )}
                        </span>
                        <span className="flex items-center gap-1.5 shrink-0">
                          {cmd.shortcut && (
                            <kbd className="text-[10px] px-1.5 py-0.5 rounded font-mono" style={kbdStyle}>{cmd.shortcut}</kbd>
                          )}
                          {active && <ChevronRight size={12} style={{ color: 'var(--color-accent)', flexShrink: 0 }} />}
                        </span>
                      </button>
                    )
                  })}
            ...
            {/* Footer */}
            <div className="flex items-center gap-3 px-4 py-2 shrink-0 text-[10px] select-none"
              style={{ borderTop: '1px solid var(--color-border)', color: 'var(--color-text-muted)' }}>
              {([['↑↓', 'navigate'], ['↵', 'run'], ['Esc', 'close']] as const).map(([key, label]) => (
                <span key={key} className="flex items-center gap-1">
                  <kbd className="px-1 py-px rounded font-mono"
                    style={{ backgroundColor: 'var(--color-surface-card)', border: '1px solid var(--color-border)' }}>{key}</kbd>
                  {label}
                </span>
              ))}
            </div>
