  ...
  <div>
    ...
    >
      ...
      <div className="space-y-2">
        <h3 className="text-sm font-semibold" style={{ color: 'var(--color-text-primary)' }}>Reporting</h3>
        <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
      ...
      <div className="space-y-2">
        <h3 className="text-sm font-semibold" style={{ color: 'var(--color-text-primary)' }}>Supported Versions</h3>
        <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
          Security fixes are only applied to the current stable release.
        </p>
      </div>
      <button
        onClick={() => window.electronAPI.openExternal('https://github.com/NeelFrostrain/UnrealLauncher/blob/main/docs/SECURITY.md')}
        className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-colors cursor-pointer"
        style={{ backgroundColor: '#dc2626', color: 'white', border: '1px solid rgba(220,38,38,0.5)' }}
      >
        <BookOpen size={16} />
