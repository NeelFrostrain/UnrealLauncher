...
interface Attachment { file: File; preview: string | null }
...
export default function FeedbackDialog({ onClose }: { onClose: () => void }): React.ReactElement {
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      ...
      attachments.forEach((a, i) => form.append(`files[${i}]`, a.file, a.file.name))
      ...
      >
            ...
            {(['report', 'suggestion'] as Mode[]).map(m => (
              <button key={m} onClick={() => { setMode(m); setStatus('idle') }}
                ...
                style={{
                  ...
                  backgroundColor: mode === m ? `color-mix(in srgb, ${accentColor} 15%, var(--color-surface-elevated))` : 'transparent',
                  color: mode === m ? accentColor : 'var(--color-text-muted)',
                  border: mode === m ? `1px solid color-mix(in srgb, ${accentColor} 25%, transparent)` : '1px solid transparent'
                }}>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-3">
          ...
          <Field label="Title *" value={title} onChange={setTitle}
            placeholder={mode === 'report' ? 'Brief description of the bug' : 'What would you like to see?'} />
          <div className="flex flex-col gap-1">
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={4}
              placeholder={mode === 'report' ? 'Steps to reproduce, expected vs actual behavior...' : 'Describe your idea in detail...'}
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => fileRef.current?.click()} disabled={attachments.length >= MAX_FILES}
                className="flex items-center gap-1 px-2 py-1 text-[10px] font-medium cursor-pointer disabled:opacity-40"
                style={{ borderRadius: 'calc(var(--radius) * 0.5)', backgroundColor: 'var(--color-surface-card)', color: 'var(--color-text-muted)', border: '1px solid var(--color-border)' }}>
                <Paperclip size={10} /> Attach
              </button>
              <input ref={fileRef} type="file" multiple accept="image/*,.pdf,.txt,.log" className="hidden"
                onChange={e => { addFiles(e.target.files); e.target.value = '' }} />
            </div>
            {fileError && <p className="text-[10px]" style={{ color: '#f87171' }}>{fileError}</p>}
            {attachments.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {attachments.map((a, i) => (
                  <div key={i} className="relative group flex items-center gap-1.5 px-2 py-1.5"
                    ...
                    <span className="text-[10px] max-w-[80px] truncate" style={{ color: 'var(--color-text-secondary)' }}>
                      {a.file.name}
                    </span>
        ...
        <div className="flex items-center gap-3 px-5 py-3.5"
          ...
          {(status === 'idle' || status === 'sending') && (
            <p className="text-[10px]" style={{ color: 'var(--color-text-muted)' }}>* required</p>
          )}
          <div className="flex items-center gap-2 ml-auto">
            <button onClick={onClose} className="px-3 py-1.5 text-xs font-medium cursor-pointer"
            ...
            <button onClick={handleSubmit} disabled={!canSubmit}
              ...
              style={{ borderRadius: 'var(--radius)', backgroundColor: accentColor, color: 'var(--color-text-primary)' }}>
              {status === 'sending'
