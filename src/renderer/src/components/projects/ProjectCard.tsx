...
import { Play, FolderOpen, Trash2, Star, Clock, Database, MoreVertical, Gamepad2, ScrollText, Copy, GitMerge, GitBranch } from 'lucide-react'
  ...
  ({
    ...
    const MenuItem = ({ icon, label, onClick, danger = false }: {
      ...
      <button
        ...
        onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = danger ? 'rgba(248,113,113,0.08)' : 'var(--color-surface-card)')}
          ...
          <div className="flex items-center gap-3 px-3 py-2.5 min-h-18">
            ...
            <div className="w-16 h-16 shrink-0 overflow-hidden flex items-center justify-center"
              ...
              {imageSrc
                ...
                : <span className="text-2xl font-black" style={{ color: 'var(--color-border)' }}>{name.charAt(0).toUpperCase()}</span>
              }
            </div>
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-1.5">
              <div className="flex items-center gap-2 flex-wrap">
                <p className="text-sm font-semibold truncate" style={{ color: 'var(--color-text-primary)' }} title={name}>{name}</p>
                ...
                {git.initialized && (
                  <span className="flex items-center gap-1 text-[9px] font-mono px-1.5 py-px shrink-0"
                    style={{ borderRadius: 'calc(var(--radius) * 0.4)', backgroundColor: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.25)', color: '#34d399' }}>
                    <GitBranch size={9} />{git.branch}
                  </span>
                )}
              </div>
            ...
            <div className="shrink-0 flex items-center gap-2 pl-3" style={{ borderLeft: '1px solid var(--color-border)' }}>
              <motion.button
                ...
                style={{ borderRadius: 'var(--radius)', backgroundColor: 'rgba(74,222,128,0.1)', border: '1px solid rgba(74,222,128,0.25)', color: '#4ade80' }}
              ...
              <div className="relative">
                ...
                <DropdownPortal open={menuOpen} anchorRef={menuBtnRef} onClose={() => setMenuOpen(false)}>
                  <MenuItem icon={<Star size={14} fill={isFavorite ? '#facc15' : 'none'} style={{ color: isFavorite ? '#facc15' : 'var(--color-text-muted)' }} />}
                    label={isFavorite ? 'Remove from Favorites' : 'Add to Favorites'}
                    onClick={() => projectPath && onToggleFavorite(projectPath)} />
                  <MenuItem icon={<FolderOpen size={14} style={{ color: 'var(--color-text-muted)' }} />} label="Open in Explorer" onClick={() => projectPath && onOpenDir(projectPath)} />
                  <MenuItem icon={<Copy size={14} style={{ color: 'var(--color-text-muted)' }} />} label="Copy Path" onClick={() => navigator.clipboard.writeText(projectPath ?? '')} />
                  <div className="h-px mx-2 my-1" style={{ backgroundColor: 'var(--color-border)' }} />
                  <MenuItem icon={<ScrollText size={14} style={{ color: 'var(--color-accent)' }} />} label="View Logs" onClick={() => setShowLogs(true)} />
                  {git.initialized
                    ? <MenuItem icon={<GitBranch size={14} style={{ color: '#34d399' }} />} label={`Git: ${git.branch}`} onClick={() => {}} />
                    : <MenuItem icon={<GitMerge size={14} style={{ color: '#a78bfa' }} />} label="Initialize Git Repo" onClick={handleGitInit} />
                  }
                  <div className="h-px mx-2 my-1" style={{ backgroundColor: 'var(--color-border)' }} />
                  <MenuItem icon={<Trash2 size={14} />} label="Remove from List" onClick={() => projectPath && onDelete(projectPath)} danger />
                </DropdownPortal>
              </div>
            </div>
          </div>
