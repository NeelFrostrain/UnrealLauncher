...
import { X, GitBranch, RefreshCw, Plus, Check, AlertTriangle, Archive, Trash2, ChevronLeft } from 'lucide-react'
...
export default function GitBranchDialog({ projectName, projectPath, currentBranch, onClose, onBranchChanged }: Props): React.ReactElement {
  ...
  const handleConflictResolve = useCallback(async (strategy: 'stash' | 'force') => {
    if (!conflict) return
    ...
    >
      ...
      >
                ...
                <div className="flex flex-col gap-2">
                  <button
                    ...
                    onMouseEnter={(e) => {
                      if (!isConflictSwitching) {
                        ...
                        e.currentTarget.style.borderColor = 'color-mix(in srgb, #60a5fa 40%, var(--color-border))'
                      }
                    }}
                    onMouseLeave={(e) => {
                  ...
                  >
                    {conflict.strategy === 'stash' && isConflictSwitching
                      ? <RefreshCw size={15} className="animate-spin shrink-0 mt-0.5" style={{ color: '#60a5fa' }} />
                    ...
                    <div>
                      ...
                      <p className="text-[11px] leading-relaxed" style={{ color: 'var(--color-text-muted)' }}>
                        Temporarily save your changes, switch branches, then restore them automatically.
                      </p>
                    </div>
                  </button>
                  ...
                  >
                    {conflict.strategy === 'force' && isConflictSwitching
                      ? <RefreshCw size={15} className="animate-spin shrink-0 mt-0.5" style={{ color: '#f87171' }} />
                          ...
                          <button
                            ...
                            onMouseEnter={(e) => {
                              if (!isCurrent && !switching) {
                                e.currentTarget.style.backgroundColor = 'var(--color-surface-elevated)'
                                e.currentTarget.style.borderColor = 'color-mix(in srgb, var(--color-accent) 30%, var(--color-border))'
                              }
                            }}
                            onMouseLeave={(e) => {
        ...
        <div className="flex items-center px-4 py-3 shrink-0"
          ...
          <button onClick={onClose} className="ml-auto px-4 py-1.5 text-xs cursor-pointer transition-colors"
            style={{
              borderRadius: 'var(--radius)',
              backgroundColor: 'var(--color-surface-card)',
