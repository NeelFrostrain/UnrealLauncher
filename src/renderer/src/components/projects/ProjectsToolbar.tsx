...
const ProjectsToolbar: FC<ProjectsToolbarProps> = ({
  tabs, currentTab, searchOpen, searchQuery,
  refreshing, backgroundScanning, calculatingSizes, addingProject,
  viewMode, sortConfig,
  onTabClick, onToggleSearch, onSearchChange,
  onAddProject, onRefresh, onViewChange, onSortChange
}) => (
  <div className="flex items-center gap-3 py-3 shrink-0 border-b" style={{ borderColor: 'var(--color-border)' }}>
        ...
        return (
          <button key={tab.id} role="tab" aria-selected={isActive}
            ...
            style={{
              ...
              backgroundColor: isActive ? 'color-mix(in srgb, var(--color-accent) 18%, var(--color-surface-elevated))' : 'transparent',
              boxShadow: isActive ? '0 1px 3px rgba(0,0,0,0.3)' : 'none'
            }}>
            {tab.icon && <span style={{ color: isActive ? 'var(--color-accent)' : 'var(--color-text-muted)' }}>{tab.icon}</span>}
            {tab.label}
          </button>
        )
      })}
    ...
    {/* Inline search */}
    {searchOpen && (
      <div className="flex items-center gap-2 px-3 py-1.5 text-xs w-48"
    ...
    <div className="flex items-center gap-1.5">
      ...
      {/* View toggle */}
      <div className="flex items-center overflow-hidden" style={{ borderRadius: 'var(--radius)', border: '1px solid var(--color-border)' }}>
        <button onClick={() => onViewChange('list')} aria-label="List view" aria-pressed={viewMode === 'list'}
        ...
        <button onClick={() => onViewChange('grid')} aria-label="Grid view" aria-pressed={viewMode === 'grid'}
          className="flex items-center p-1.5 cursor-pointer transition-colors"
      ...
      {/* Search toggle */}
      <button onClick={onToggleSearch} aria-label={searchOpen ? 'Close search' : 'Open search'} aria-pressed={searchOpen}
        className="flex items-center p-1.5 cursor-pointer transition-colors"
        style={{ borderRadius: 'var(--radius)', backgroundColor: searchOpen ? 'color-mix(in srgb, var(--color-accent) 20%, transparent)' : 'var(--color-surface-card)', color: searchOpen ? 'var(--color-accent)' : 'var(--color-text-muted)', border: '1px solid var(--color-border)' }}
        title="Search">
        <Search size={13} />
      </button>
      ...
      <div className="relative">
        <button onClick={onRefresh} disabled={refreshing}
          ...
          {refreshing ? 'Refreshing…' : calculatingSizes ? 'Calculating sizes…' : backgroundScanning ? 'Scanning…' : 'Refresh'}
        </button>
        {calculatingSizes && (
          <span className="absolute -top-2 -right-2 px-2 py-0.5 text-[10px] font-semibold rounded-full"
            style={{ backgroundColor: 'var(--color-accent)', color: 'var(--color-text-primary)', boxShadow: '0 2px 6px rgba(0,0,0,0.35)' }}>
            Sizes
          </span>
        )}
      </div>
      ...
      {/* Add Project */}
      <button onClick={onAddProject} disabled={addingProject}
