...
export function useFindBar(content: string, setContent: (v: string) => void, onToast: (msg: string, type: 'success' | 'error' | 'info') => void) {
  const [find, setFind] = useState<FindState>(INITIAL)
  const findInputRef    = useRef<HTMLInputElement>(null)
  const replaceInputRef = useRef<HTMLInputElement>(null)
  const textareaRef     = useRef<HTMLTextAreaElement>(null)

  const matches     = useMemo(() => getMatches(content, find.query, find.caseSensitive), [content, find.query, find.caseSensitive])
  ...
  const replaceOne = useCallback(() => {
    ...
    setContent(content.substring(0, pos) + find.replaceQuery + content.substring(pos + find.query.length))
  ...
  return {
    find, setFind,
    matches, matchCount, currentMatch,
