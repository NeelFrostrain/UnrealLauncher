...
import { FolderOpen, RefreshCw, Package, Search, FolderInput, LayoutGrid, LayoutList } from 'lucide-react'
...
const FabTab = (): React.ReactElement => {
  ...
  useEffect(() => {
    const init = async (): Promise<void> => {
      ...
      if (saved) { setFolderPath(saved); scan(saved) }
      else {
  ...
  const scan = useCallback(async (dir: string): Promise<void> => {
    if (!dir) return
    setLoading(true)
    try { setAssets(await window.electronAPI.fabScanFolder(dir)) }
    catch { setAssets([]) }
    setLoading(false)
  }, [])
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 border-b shrink-0"
        ...
        <button onClick={handlePickFolder}
          ...
          style={{ borderRadius: 'var(--radius)', backgroundColor: 'var(--color-surface-card)', border: '1px solid var(--color-border)', color: folderPath ? 'var(--color-text-secondary)' : 'var(--color-text-muted)', transition: 'border-color 150ms ease' }}
        ...
        {/* Search */}
        <div className="flex items-center gap-2 px-3 py-1.5 text-xs w-44 shrink-0"
        ...
        {/* Refresh */}
        <button onClick={() => scan(folderPath)} disabled={loading || !folderPath}
      ...
      {/* ── Filter pills ── */}
      {assets.length > 0 && (
        <FabFilterBar assets={assets} typeFilter={typeFilter} typeLabels={TYPE_LABELS} onFilterChange={setTypeFilter} />
      )}
        ...
        {!folderPath ? (
          <div className="flex flex-col items-center justify-center h-full py-8 px-6">
            ...
            <div className="w-12 h-12 flex items-center justify-center rounded-xl mb-4"
              style={{ backgroundColor: 'color-mix(in srgb, var(--color-accent) 12%, transparent)', border: '1px solid color-mix(in srgb, var(--color-accent) 20%, transparent)' }}>
            ...
            <p className="text-xs mb-6 text-center max-w-xs" style={{ color: 'var(--color-text-muted)' }}>
            ...
            <div className="w-full max-w-sm flex flex-col gap-2 mb-6">
              {[
                ...
                <div key={n} className="flex items-start gap-3 px-3 py-2.5"
                  style={{ borderRadius: 'var(--radius)', backgroundColor: 'var(--color-surface-card)', border: '1px solid var(--color-border)' }}>
                  <span className="shrink-0 w-5 h-5 flex items-center justify-center text-[10px] font-bold rounded-full"
                    style={{ backgroundColor: 'color-mix(in srgb, var(--color-accent) 18%, transparent)', color: 'var(--color-accent)', border: '1px solid color-mix(in srgb, var(--color-accent) 25%, transparent)' }}>
                    {n}
                  </span>
                  <span className="text-xs leading-relaxed" style={{ color: 'var(--color-text-secondary)' }}>{text}</span>
                </div>
              ))}
            </div>
          ...
          <div className="grid gap-2 pt-2"
            ...
            {filtered.map((asset) => <AssetGridCard key={asset.folderPath} asset={asset} />)}
          </div>
        ) : (
          <div className="flex flex-col gap-1.5 pt-2">
            {filtered.map((asset) => <AssetListCard key={asset.folderPath} asset={asset} />)}
          </div>
        )}
      </div>
    </div>
