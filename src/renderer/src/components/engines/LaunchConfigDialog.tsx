...
import {
  X, Play, Plus, Trash2, ChevronDown,
  Cpu, Layers, Settings2, Zap, Pencil,
  Check, Copy, Terminal
} from 'lucide-react'
...
const UE_DEFAULTS: Omit<LaunchConfig, 'id' | 'name' | 'description'> = {
  rhi: 'default', scalability: 'default',
  lumen: true, nanite: true, vsm: true, rayTracing: false,
  ssr: true, taa: true, bloom: true, ambientOcclusion: true,
  motionBlur: true, lensFlare: true, autoExposure: true, depthOfField: true,
  noSplash: false, noLoadingScreen: false, noShaderCompile: false,
  unattended: false, extraArgs: ''
}

const SCAL_COLORS: Record<string, string> = {
  default: 'var(--color-text-muted)',
  '0': '#f87171', '1': '#fb923c', '2': '#facc15', '3': '#4ade80', '4': '#818cf8'
}
const SCAL_LABELS: Record<string, string> = {
  default: 'Default', '0': 'Low', '1': 'Medium', '2': 'High', '3': 'Epic', '4': 'Cinematic'
}
const RHI_LABELS: Record<RHI, string> = {
  default: 'Default', dx11: 'DirectX 11', dx12: 'DirectX 12', vulkan: 'Vulkan', opengl: 'OpenGL'
}
...
function Toggle({ on, onChange, disabled = false }: { on: boolean; onChange: () => void; disabled?: boolean }) {
  return (
    <button
      ...
      style={{
        backgroundColor: on ? 'var(--color-accent)' : 'color-mix(in srgb, var(--color-border) 120%, transparent)',
        boxShadow: on ? '0 0 0 2px color-mix(in srgb, var(--color-accent) 30%, transparent)' : 'none'
      }}
      role="switch"
      aria-checked={on}
    >
...
// ── Feature row ───────────────────────────────────────────────────────────────
function FeatureRow({
  label, sub, value, onChange, disabled = false, warn = false
}: {
  label: string; sub?: string; value: boolean; onChange: () => void; disabled?: boolean; warn?: boolean
}) {
  return (
    <div
      className="flex items-center justify-between gap-4 px-4 py-3.5 transition-colors"
      style={{
        backgroundColor: !disabled && value && warn
    ...
    >
      <div className="min-w-0 flex-1">
        ...
        {sub && (
          <p className="text-[11px] mt-0.5 font-mono leading-snug" style={{ color: 'var(--color-text-muted)' }}>
            {sub}
          </p>
        )}
      </div>
...
// ── Section header ────────────────────────────────────────────────────────────
function SectionHead({ icon, label, accent = 'var(--color-accent)' }: { icon: React.ReactNode; label: string; accent?: string }) {
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-4 shrink-0"
          ...
          <button onClick={onClose} className="p-2 cursor-pointer transition-colors"
            style={{
              borderRadius: 'calc(var(--radius) * 0.5)',
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div className="w-64 shrink-0 flex flex-col"
            ...
            <div className="px-4 py-3 shrink-0"
              style={{ borderBottom: '1px solid var(--color-border)' }}>
              <p className="text-[11px] font-semibold uppercase tracking-widest select-none"
              ...
              {configs.map((cfg) => {
                ...
                return (
                  <div key={cfg.id}
                    ...
                    onClick={() => { if (!isRenaming) { setSelectedId(cfg.id); setEditing(null) } }}
                  >
                    {isActive && (
                    ...
                    {isCustom && !isRenaming && (
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                        <button onClick={(e) => { e.stopPropagation(); setRenamingId(cfg.id); setRenameValue(cfg.name); setTimeout(() => renameRef.current?.select(), 30) }}
                          className="p-1 cursor-pointer rounded" title="Rename"
                          style={{ color: 'var(--color-text-muted)' }}>
                          <Pencil size={12} />
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); deleteConfig(cfg.id) }}
                          className="p-1 cursor-pointer rounded" title="Delete"
                          style={{ color: 'var(--color-text-muted)' }}
              ...
              {showNewForm ? (
                <div className="flex flex-col gap-2">
                  <input autoFocus value={newName}
                    ...
                    onKeyDown={(e) => { if (e.key === 'Enter') createNew(); if (e.key === 'Escape') setShowNewForm(false) }}
                  ...
                  <div className="flex gap-1.5">
                    <button onClick={createNew}
                      ...
                      style={{ borderRadius: 'calc(var(--radius) * 0.6)', backgroundColor: 'var(--color-accent)', color: 'white' }}>
            ...
            <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
              ...
              <div className="flex items-center justify-between gap-3 px-5 py-3.5 shrink-0"
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {!editing && !isBuiltIn && (
                    <button onClick={() => startEdit(display)}
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4">
                ...
                <div className="overflow-hidden"
                  ...
                  <div className="flex items-center justify-between gap-4 px-4 py-4">
                    ...
                    <StyledSelect value={editing ? editing.rhi : display.rhi}
                      onChange={(v) => patch({ rhi: v as RHI })} disabled={!editing}>
                ...
                <div className="overflow-hidden"
                  ...
                  <div className="flex items-center justify-between gap-4 px-4 py-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <StyledSelect value={scalStr}
                        onChange={(v) => patch({ scalability: v === 'default' ? 'default' : (Number(v) as Scalability) })}
                        disabled={!editing}>
                        <option value="default">Default (no override)</option>
                ...
                <div className="overflow-hidden divide-y"
                  ...
                  ] as [keyof LaunchConfig, string, string, boolean][]).map(([key, label, cvar, isHeavy]) => (
                ...
                <div className="overflow-hidden divide-y"
                  ...
                  ] as [keyof LaunchConfig, string, string][]).map(([key, label, flag]) => (
        ...
        <div className="flex items-center justify-between gap-4 px-5 py-4 shrink-0"
            ...
            {selected && (
              <>
                ...
                {selected.scalability !== 'default' && (
                  <Pill label={SCAL_LABELS[String(selected.scalability)]} color={SCAL_COLORS[String(selected.scalability)]} />
                )}
              </>
            )}
          </div>
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            <button onClick={handleLaunch} disabled={launching || !selected}
              ...
              style={{
                ...
                color: 'white',
                boxShadow: launching ? 'none' : '0 4px 14px color-mix(in srgb, var(--color-accent) 35%, transparent)'
              }}>
