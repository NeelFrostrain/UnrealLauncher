...
const FONT_OPTIONS = [
  { id: 'inter',     label: 'Inter',     value: "'Inter', system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" },
  { id: 'open-sans', label: 'Open Sans', value: "'Open Sans', system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" },
  { id: 'outfit',    label: 'Outfit',    value: "'Outfit', system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" },
  { id: 'roboto',    label: 'Roboto',    value: "'Roboto', system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" },
  { id: 'rubik',     label: 'Rubik',     value: "'Rubik', system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" },
  { id: 'ubuntu',    label: 'Ubuntu',    value: "'Ubuntu', system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }
]
...
const FontControls = ({ activeThemeId, customOverrides, setOverride }: FontControlsProps): React.ReactElement => {
  ...
  const currentFontSizeRaw = customOverrides['font-size'] ?? activeThemeTokens?.['font-size'] ?? '15px'
    ...
    <>
      ...
      <div className="p-5" style={{ borderTop: '1px solid var(--color-border)' }}>
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-sm font-medium" style={{ color: 'var(--color-text-primary)' }}>Font family</p>
        ...
        <div className="grid gap-2" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))' }}>
          {FONT_OPTIONS.map((option) => {
            const active =
              ...
              (customOverrides['font-family'] === undefined && activeThemeTokens?.['font-family'] === option.value)
            return (
              <button
                key={option.id}
                type="button"
      ...
      <div className="p-5" style={{ borderTop: '1px solid var(--color-border)' }}>
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-sm font-medium" style={{ color: 'var(--color-text-primary)' }}>Font size</p>
            <p className="text-xs mt-0.5" style={{ color: 'var(--color-text-muted)' }}>
              Adjust base UI text size for readability.
            </p>
          </div>
          <span className="text-xs font-mono" style={{ color: 'var(--color-text-muted)' }}>{currentFontSize}px</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[10px]" style={{ color: 'var(--color-text-muted)' }}>Smaller</span>
          <input
            type="range" min={12} max={20} step={1} value={currentFontSize}
